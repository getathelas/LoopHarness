...
final class SettingsVC: UIViewController {
    ...
    /// Managed builds pin the backend and lock down skills, so drop those rows
    /// (and any section left empty — e.g. "Core") from Settings. The SSH panel
    /// stays visible so a managed user can inspect the connection baked into
    /// their build. Model switching stays available too: it routes to the same
    /// ModelPickerVC, which gates picks on a configured key, so a managed user
    /// can still move between the models their build can actually run.
    private func hidingManagedRows(_ sections: [Section]) -> [Section] {
        let hidden: Set<String> = ["Execution Backend", "Skills"]
