...
//  Loop
//
//  Direct client-side Tinker inference path. Used by AgentHarness when
//  the selected model's provider is `.thinky` and the user has a
//  THINKY_API_KEY set in the Keys panel (or bundled via
//  Secrets.xcconfig). Talks straight to Tinker's OpenAI-compatible
//  chat/completions endpoint with the user's own key.
//
...
//    • endpoint (tinker.thinkingmachines.dev instead of api.openai.com)
//    • model id (the user's tinker:// sampler checkpoint path)
//    • error domain so a failure surfaces as "Tinker API error: …"
//

import Foundation

final class TinkerChat {

    static let shared = TinkerChat()
    private init() {}
    ...
    private let endpoint = URL(string: "https://tinker.thinkingmachines.dev/services/tinker-prod/oai/api/v1/chat/completions")!

    // MARK: - Prompt caching
    //
    // Tinker supports automatic prefix caching on their OpenAI-
    ...
    // → latest user message. Tinker does not currently expose a
    ...
    func chat(messages: [MessageStruct],
        ...
        guard let apiKey = KeyStore.shared.value(for: .thinky),
              !apiKey.isEmpty else {
            completion(nil, Self.error(
                "Thinking Machines is selected but no THINKY_API_KEY is set. Add THINKY_API_KEY in Settings ▸ Keys or switch the model in Settings ▸ Model."))
            return
        }

        // `modelIDOverride` lets a caller pin a specific Tinker model
        // regardless of the user's selection — used by the per-turn vision
        // fallback when a future vision-capable model is selected.
        let modelID = modelIDOverride ?? ModelSelectionStore.current.apiModelID
        guard let modelID, !modelID.isEmpty else {
            completion(nil, Self.error(
                "Thinking Machines is selected but no model path is set. Add THINKY_MODEL_PATH in Settings ▸ Keys."))
            return
        }
        ...
        guard let payload = try? JSONSerialization.data(withJSONObject: body) else {
            completion(nil, Self.error("Failed to encode the Tinker request body."))
            return
        }

        var metrics = InferenceMetrics(provider: "Tinker", model: modelID, toolCount: (tools ?? []).count)
        ...
        let reader = SSEStreamReader(metrics: metrics, onDelta: onPartial) { result in
            ...
            case .failure(let error):
                completion(nil, Self.error("Tinker streaming error: \(error.localizedDescription)"))
            }
        }
    ...
    private static func error(_ message: String) -> NSError {
        NSError(domain: "TinkerChat", code: -1,
